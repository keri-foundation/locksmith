...
class DeleteReceivedCredentialDialog(LocksmithResourceDeletionDialog):
    ...
    def __init__(self, schema_name: str, said: str, icon_path: str, app, parent: "VaultPage" = None):
        """
        Initialize the DeleteReceivedCredentialDialog.
        ...
        super().__init__(
            parent=parent,
            resource_type="received credential",
        ...
        logger.info(f"DeleteReceivedCredentialDialog initialized for '{schema_name}' ({said})")

    def _delete_credential(self):
        """Handle the delete button click and perform credential deletion."""
        logger.info(f"Attempting to delete received credential '{self.schema_name}' ({self.said})")

        try:
            # Remove the credential from this wallet
            credentialing.delete_credential(self.app.vault, self.said, received=True)
            ...
            if hasattr(self.app.vault, 'signals') and self.app.vault.signals:
                self.app.vault.signals.emit_doer_event(
                    doer_name="DeleteReceivedCredential",
            ...
            logger.info(f"Received credential '{self.schema_name}' deleted successfully")
            self.accept()
