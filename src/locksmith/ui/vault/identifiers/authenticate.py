# -*- encoding: utf-8 -*-
"""
locksmith.ui.vault.identifiers.authenticate module

Dialog for authenticating witnesses during identifier rotation
"""
...
class WitnessAuthenticationDialog(WitnessAuthenticationMixin, LocksmithDialog):
    """Dialog for authenticating witnesses during rotation."""

    def __init__(self, app, hab, witness_ids: list[str], auth_only=False, signals=None, parent=None):
        """
        Initialize the WitnessAuthenticationDialog.
        ...
        Args:
            ...
            signals: Optional signals object to emit auth codes instead of authenticating directly
        ...
        # Use finished signal for reliable cleanup — closeEvent is not called when accept() is used
    ...
    def _on_doer_event(self, doer_name: str, event_type: str, data: dict):
        ...
        """
        logger.info(f"WitnessAuthenticationDialog received doer_event: {doer_name} - {event_type}")
